...
class Profile_Magic_Basic_Functions 
{
    ...
    public function pg_get_extension_button($plugin)
    {
        ...
        if($data->is_pg_extension_activate)
        {
            if($plugin !='FRONTEND_GROUP_MANAGER' && $plugin !='MULTI_ADMINS' && $plugin !='EVENTS' && $plugin !='MENU_RESTRICTIONS' && $plugin !='HERO_BANNER'):
        ?>
            <li>
            <a class="install-now button pg-install-now-btn pg-more-options" href="<?php echo $optionurl;?>"><?php _e("OPTIONS","profilegrid-user-profiles-groups-and-communities");?></a>
            </li>
        <?php
            endif;
        }
        elseif($data->is_pg_extension_installed)
        {
            ?>
            <li>
            <a class="install-now button pg-install-now-btn pg-active-now" href="<?php echo $data->pg_extsion_activate_url;?>"><?php _e("Activate Now","profilegrid-user-profiles-groups-and-communities");?></a>
            </li>
        <?php
        }
        else
        {
            if($plugin=='CUSTOM_PROFILE_SLUG' || $plugin=='DISPLAY_NAME' || $plugin=='BBPRESS' || $plugin=='WOOCOMMERCE' || $plugin=='EVENTS' || $plugin=='LOGIN_LOGOUT_MENU' || $plugin=='DEMO_CONTENT' || $plugin=='HERO_BANNER')
            {
                $title = __('Free','profilegrid-user-profiles-groups-and-communities');
                $class = 'pg-free-extension';
                ?>
            <li class="<?php echo $class;?>-wrap">
            ...
            ?>
            <li>
            <a class="install-now button pg-install-now-btn pg-more-info" target="_blank" href="<?php echo $data->pg_get_extension_install_url;?>"><?php echo __('More Info','profilegrid-user-profiles-groups-and-communities');?></a>
            </li>
            <?php 
        }
    }

...
class Profile_Magic_Public {
        ...
        public function profile_magic_login_notice( $message ) 
        {
            ...
            $pmrequests = new PM_request;
                ...
                if (isset($_GET['pgr']) && $_GET['pgr']==1) 
                {
                    $dbhandler = new PM_DBhandler;
                    $allowed = $dbhandler->get_global_option_value('pm_guest_allow_backend_login_screen','1');
                    if($allowed==0)
                    {
                        $message =  '<div class="message">' .__('Dashboard login disabled for Guests users. Try logging in using log in form on the website.','profilegrid-user-profiles-groups-and-communities'). '</div>';
                    }
                }
    ...
    if(!empty($users))
    {
        ...
        { 
            if($user->ID!=$uid)
            {
                ...
                $user_info['label']=  strip_tags($pmrequests->pm_get_display_name($user->ID,true));
                $return[]=$user_info;
            }
        }
    }
...
//                  'menu_icon' =>'dashicons-testimonial'
        ...
        public function pm_show_friends_tab($uid,$gid)
	{
            ...
            {
                echo '<li class="pm-profile-tab pg-friend-tab pm-pad10"><a class="pm-dbfl" href="#pg-friends">'. __('Friends','profilegrid-user-profiles-groups-and-communities').'</a></li>';
            }
	}
    ...
    public function pg_rm_registration_tab($uid,$gid)
    {
        ...
        {
            echo '<li class="pm-dbfl pg-rm-registration-tab pm-border-bt pm-pad10"><a class="pm-dbfl" href="#pg_rm_registration_tab">'.$title.'</a></li>';
        }
    }
    ...
    public function pm_profile_about_tab_content($id,$tab,$uid,$gid,$primary_gid)
    {
      ...
      <?php if($uid == $current_user->ID && $dbhandler->get_global_option_value('pm_show_user_edit_profile_button','1')=='1'):
...
<div class="pm-section-left-panel pm-section-no-left-panel pm-section-nav-vertical pm-difl pm-border pm-radius5 pm-bg">
    ...
    public function profile_magic_shortcode_user_image($content)
    {
        ...
        echo get_avatar($useremail,150,'',false,array('class'=>'pm-user','force_display'=>true));
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_display_name($content)
    {
        ...
        $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
        if($attributes['uid']!==0 && is_object($user_exists)):
        echo $pmrequests->pm_get_display_name($attributes['uid'],true);
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_first_name($content)
    {
        ...
        $attributes = shortcode_atts( $default_attributes, $content );
          $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
          if($attributes['uid']!==0 && is_object($user_exists)):
        echo  $pmrequests->profile_magic_get_user_field_value($attributes['uid'],'first_name');
         endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_last_name($content)
    {
        ...
        $attributes = shortcode_atts( $default_attributes, $content );
         $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
         if($attributes['uid']!==0 && is_object($user_exists)):
        echo  $pmrequests->profile_magic_get_user_field_value($attributes['uid'],'last_name');
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_email($content)
    {
        ...
        $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
         if($attributes['uid']!==0 && is_object($user_exists)):
        echo  $pmrequests->profile_magic_get_user_field_value($attributes['uid'],'user_email');
         endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_cover_image($content)
    {
        ...
        $attributes = shortcode_atts( $default_attributes, $content );
         $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
         if($attributes['uid']!==0 && is_object($user_exists)):
        echo  $pmrequests->profile_magic_get_cover_image($attributes['uid'],'pm-cover-image');
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_default_group($content)
    {
        ...
        $attributes = shortcode_atts( $default_attributes, $content );
         $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
         if($attributes['uid']!==0 && is_object($user_exists)):
        echo  $pmrequests->profile_magic_get_user_field_value($attributes['uid'],'group_name');
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_all_groups($content)
    {
        ...
        ob_start();
        $current_user = wp_get_current_user();
        $default_attributes = array( 'uid' => $current_user->ID,'sep'=>', ');
        $attributes = shortcode_atts( $default_attributes, $content );
         $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
         if($attributes['uid']!==0 && is_object($user_exists)):
        $sep = $attributes['sep'];
    ...
    public function profile_magic_shortcode_user_group_badges($content)
    {
        ...
        echo $pmrequests->pg_get_user_groups_badge_slider($attributes['uid']);
        echo '</div></div>';
        endif;
        $html = ob_get_contents();
        ob_end_clean();
    ...
    public function profile_magic_shortcode_user_unread_notifications_count($content)
    {
        ...
        $user_exists = $pmrequests->pm_check_user_exist_by_id($attributes['uid']);        
        if($attributes['uid']!==0 && is_object($user_exists)):
            echo $unread_notification = $pm_notification->pm_get_user_unread_notification_count($attributes['uid']);
        endif;
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
    ...
    public function profile_magic_shortcode_user_friends_area($content)
    {
        ...
        echo '<div class="pmagic"><div class="pm-group-view pg-shortcode-content">';
        echo '<input type="hidden" name="pm-uid" id="pm-uid" value="'.$attributes['uid'].'" />';
     ...
     public function profile_magic_shortcode_user_delete_account_tab($content)
    {
        ...
        {
            $delete_error = __("You entered incorrect password. Please try again.",'profilegrid-user-profiles-groups-and-communities');
        }
    ...
    public function profile_magic_shortcode_group_description($content)
    {
        ...
        if(!empty($attributes['gid']) && strpos($attributes['gid'],',')==false):
        $groupinfo = $dbhandler->get_row('GROUPS',$attributes['gid']);
        if(isset($groupinfo)){$value = $groupinfo->group_desc;}else{$value = '';}
        echo $value;
        endif;
        $html = ob_get_contents();
        ob_end_clean();
    ...
    public function profile_magic_shortcode_group_manager_display_name($content)
    {
        ...
        $name = array();
        ob_start();
        $default_attributes = array( 'gid' => 0 ,'sep'=>', ' );
        $attributes = shortcode_atts( $default_attributes, $content );
        $gid = $attributes['gid'];
        $sep = $attributes['sep'];
        if(!empty($attributes['gid']) && strpos($attributes['gid'],',')==false):
        ...
        if(!empty($name))
        {
            ...
            echo  implode($sep,array_unique($name));
        }
        endif;
        $html = ob_get_contents();
        ob_end_clean();
    ...
    public function profile_magic_shortcode_group_manager_display_name_in_list($content)
    {
        ...
        if(!empty($leaders))
        {
            ...
            foreach($leaders as $leader)
            {
                echo '<li>'. $pmrequests->pm_get_display_name($leader).'</li>';
            }
            echo '</ul>';
        }
        endif;
	...
	$total_users = $user_query->get_total();
        ...
        if(!empty($users))
        {
            ...
            foreach($users as $user)
            {
                echo '<li>'. $pmrequests->pm_get_display_name($user->ID).'</li>';
            }
            echo '</ul>';
        }
        endif;
	...
	$total_users = $user_query->get_total();
        $users = $user_query->get_results();
        $num_of_pages = ceil( $total_users/$limit);
        //$base = get_permalink().'?gid='.$gid.'&pagenum='.;
	$pagination = $dbhandler->pm_get_pagination($num_of_pages,$pagenum);
        $leaders = $pmrequests->pg_get_group_leaders($gid);
...
?>
    ...
    public function profile_magic_shortcode_content_visible($args,$content)
    {
        ...
        $pmrequests = new PM_request;
        $current_user = wp_get_current_user();
        $default_attributes = array('gid'=>'','min_blog'=>'','min_wc_spent'=>'','min_edd_spent'=>'');
        ...
        if(!empty($check) && is_user_logged_in())
        {
            ...
            //return $attributes['minimum_total_edd_spent'];die;

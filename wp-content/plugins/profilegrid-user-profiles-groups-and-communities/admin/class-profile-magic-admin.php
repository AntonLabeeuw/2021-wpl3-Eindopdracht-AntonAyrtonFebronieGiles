...
//            if($is_pg_dashboard_page):
		...
		wp_localize_script( $this->profile_magic, 'pm_error_object',$error);
                ...
                $upload_requirements['error_min_profile_width'] = sprintf( __( 'Image dimensions are too small. Minimum size is %s by %s pixels.','profilegrid-user-profiles-groups-and-communities' ),$upload_requirements['pg_profile_photo_minimum_width'] ,$upload_requirements['pg_profile_photo_minimum_width']);
                $upload_requirements['error_min_cover_width'] = sprintf( __( 'Image dimensions are too small. Minimum size is %s by 300 pixels.','profilegrid-user-profiles-groups-and-communities' ),$upload_requirements['pg_cover_photo_minimum_width']);
   ...
   ?>
		...
		if($lastRec=count($fields) && !empty($gid))
		{
			...
			echo '</div>';
                ...
                if(is_object($user))
                {
                    echo '<input type="hidden" id="pg_user_id" name="pg_user_id" value="'.$user->ID.'" />';
                }
	...
	if($row->group_options!="")
        ...
        $arg = array('post_type' => 'page','post_title' =>$group_name,'post_status' => 'publish','post_content' => '[profilegrid_group gid="'.$gid.'"]');
     ...
     public function pm_remove_file_attachment()
    {
        ...
        {
            echo delete_user_meta($userid, $key);
        }
        else
        {
            $ids = implode(',', $old_attachments);
            echo update_user_meta($userid, $key, $ids);
